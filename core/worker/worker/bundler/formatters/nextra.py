...
def _ensure_index_page(
    ...
    lines.append("description: Generated documentation")
    lines.append("---")
    lines.append("")
    ...
    for slug, data in navigation.items():
        ...
        if children:
            ...
            for child_id, child_data in children.items():
                ...
                if child_id in pages and pages[child_id].title:
...
def _generate_meta_files(
    ...
    for nav_id, nav_data in navigation.items():
        ...
        if isinstance(nav_data, dict) and "children" in nav_data:
            ...
            # Add each child to the group's _meta.js.
            ...
            for child_id, child_data in children.items():
                if isinstance(child_data, dict):
                    ...
                    child_title = str(child_data)
                # Prefer the actual stored page title.
                if child_id in pages and pages[child_id].title:
                    child_title = pages[child_id].title
                add_page(Path(nav_id), child_id, child_title)
        else:
            # Plain page — must have a corresponding .mdx file.
            title: str
            ...
            # Prefer the actual page title if available.
            if nav_id in pages and pages[nav_id].title:
