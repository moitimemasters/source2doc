...
from source2doc.models.review import CriticOutput
...
def create_critic_agent(
...
) -> tuple[pydantic_ai.Agent[DocGenDeps, CriticOutput], AgentConfig]:
    prompt_config = load_prompt(prompt_path)
    model = create_llm_model(llm_config)

    instructions = prompt_config.instructions + build_directive(output_language, "critic")
    ...
    search_code_retries = prompt_config.tool_retries.get("search_code", 2)
