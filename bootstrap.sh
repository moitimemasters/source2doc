...
# Optionally generate machine-specific extra_hosts pins for endpoints
# whose DNS only resolves through a VPN resolver Docker Desktop can't see
# (corp DNS, dedicated internal zones, etc.). Picks AAAA first (modern
# corp stacks tend to be v6-only), falls back to A. Falls through silently
# under NONINTERACTIVE.
resolve_host_address() {
    local host=$1 addr=""
    if command -v dig >/dev/null 2>&1; then
        addr=$(dig +short +time=2 +tries=1 AAAA "$host" 2>/dev/null | grep -E '^[0-9a-fA-F:]+$' | head -1)
        if [ -z "$addr" ]; then
            addr=$(dig +short +time=2 +tries=1 A "$host" 2>/dev/null | grep -E '^[0-9.]+$' | head -1)
        fi
    fi
    if [ -z "$addr" ] && command -v host >/dev/null 2>&1; then
        addr=$(host -t AAAA "$host" 2>/dev/null | awk '/has IPv6 address/ {print $NF; exit}')
        if [ -z "$addr" ]; then
            addr=$(host -t A "$host" 2>/dev/null | awk '/has address/ {print $NF; exit}')
        fi
    fi
    if [ -z "$addr" ] && command -v getent >/dev/null 2>&1; then
        addr=$(getent ahostsv6 "$host" 2>/dev/null | awk '{print $1; exit}')
        if [ -z "$addr" ]; then
            addr=$(getent ahosts "$host" 2>/dev/null | awk '{print $1; exit}')
        fi
    fi
    echo "$addr"
}

# Service names that need outbound access to VPN-resolved hostnames.
# Adjust here if more services start making outbound LLM/embedder calls.
EXTRA_HOSTS_SERVICES=(gateway worker-docgen worker-codetour worker-bundler)

# Stores collected host pins as "host=addr" lines in a single string,
# separated by newlines. Bash 3.2 (default on macOS) has flaky array
# handling under `set -u`, so we keep it as a plain string and split
# with IFS=$'\n' at write time.
EXTRA_HOST_PAIRS=""
EXTRA_HOSTS_FILE="docker-compose.override.yml"

if [ "${NONINTERACTIVE:-0}" != "1" ] && [ -t 0 ] \
    && [ "$(ask_yes_no "Pin extra_hosts for VPN-internal endpoints (skip if Docker can resolve them already)?" n)" = "y" ]; then
    while true; do
        read -r -p "    Hostname (blank to finish): " _host </dev/tty
        _host=${_host// /}
        [ -z "$_host" ] && break
        _addr=$(resolve_host_address "$_host")
        if [ -z "$_addr" ]; then
            echo "    couldn't resolve $_host — check the VPN is up and try again, or paste an IP manually" >&2
            read -r -p "    Manual IP for $_host (blank to skip): " _addr </dev/tty
            _addr=${_addr// /}
            [ -z "$_addr" ] && continue
        else
            echo "    resolved $_host -> $_addr"
        fi
        EXTRA_HOST_PAIRS="${EXTRA_HOST_PAIRS}${_host}=${_addr}"$'\n'
    done
fi

if [ -n "$EXTRA_HOST_PAIRS" ]; then
    if [ -f "$EXTRA_HOSTS_FILE" ] && [ "$(ask_yes_no "$EXTRA_HOSTS_FILE exists — overwrite?" n)" != "y" ]; then
        echo "  keeping existing $EXTRA_HOSTS_FILE — not writing extra_hosts"
    else
        {
            echo "# Generated by ./bootstrap.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)."
            echo "# Pins AAAA/A records for hostnames Docker Desktop's resolver can't see"
            echo "# (typically corporate VPN endpoints). Refresh with ./bootstrap.sh on"
            echo "# IP rotation, or hand-edit and re-up."
            echo "services:"
            for svc in "${EXTRA_HOSTS_SERVICES[@]}"; do
                echo "  $svc:"
                echo "    extra_hosts:"
                printf '%s' "$EXTRA_HOST_PAIRS" | while IFS= read -r _pair; do
                    [ -z "$_pair" ] && continue
                    _h=${_pair%%=*}
                    _a=${_pair#*=}
                    echo "      - \"$_h:$_a\""
        ...
        _count=$(printf '%s' "$EXTRA_HOST_PAIRS" | grep -c '=')
        echo "  wrote $EXTRA_HOSTS_FILE with $_count host pin(s)"
    fi
fi
